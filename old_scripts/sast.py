...
try:
    ...
    print("ERROR: Ollama library not found. Please install it with 'pip install ollama'.")
    exit(1)

# Supported file extensions for analysis
EXTENSIONS = {".py", ".js", ".ts", ".go", ".java", ".cpp", ".c", ".rs", ".php", ".rb"}

def ask_model_with_retries(prompt, model, max_attempts=3, wait_seconds=1):
    """
...
def main():
    ...
    parser.add_argument("--model", "-m", default="hengwen/DeepSeek-R1-Distill-Qwen-32B:q4_k_m", 
                        help="Name of the Ollama model to use (default: DeepSeek-R1-Distill-Qwen-32B:q4_k_m)")
    ...
    analysis_results = {}  # will hold results for each file
    ...
    for file_path in code_files:
        ...
        try:
            response1 = ollama.chat(model=model_name, messages=[{"role": "user", "content": prompt_stage1}])
            output1 = response1.get("message", {}).get("content", "")  # the text output from model
        except Exception as e:
        ...
        try:
            response2 = ollama.chat(model=model_name, messages=[{"role": "user", "content": prompt_stage2}])
            output2 = response2.get("message", {}).get("content", "")
        except Exception as e:
    ...
    summary_md = ""
    try:
        response3 = ollama.chat(model=model_name, messages=[{"role": "user", "content": summary_prompt}])
        summary_md = response3.get("message", {}).get("content", "")
    except Exception as e:
